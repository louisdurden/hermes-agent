"""Opt-in, bounded Graph-of-Agents coordination for one Hermes turn."""
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "You are a tool-free GoA coordinator. Return only the "
                        "requested concise analysis."
                    ),
                },
                {"role": "user", "content": prompt},
            ],
            max_tokens=_STAGE_TOKENS[stage],
            reasoning_config={"effort": meta.get("reasoning_effort", "medium")},
            tools=[],
        )
    )
...
def _profile_session(profile: str) -> tuple[dict[str, Any], str]:
    ...
    try:
        ...
        logger.warning("GoA profile %s could not be resolved; using fallback runtime", profile)
        return fallback, ""
...
def _profile_call(
    ...
    *,
    ...
    prompt = (
        f"You are Hermes profile {profile}. Work within its specialty. No tools.\n"
        f"Profile session identity:\n{soul}\nTask: {question}\nOwn draft: {own}\n"
        f"Peer material:\n{refs}\nReturn a concise {stage}."
    )
    return _call(runtime, prompt, stage=stage)


def aggregate_guidance(*, user_prompt: str, config: dict[str, Any]) -> str:
    """Run one bounded graph; disabled input is always a no-op."""
    ...
    card_text = "\n".join(
        f"{card.agent_id}: {card.domain}; {', '.join(card.specializations)}"
        for card in CARDS
        if card.agent_id in permitted
    )
    selection = _call(
        meta,
        f'Select exactly {config["top_k"]} relevant profile ids as JSON '
        f'{{"selected":[...]}}. Never select fleetfabric.\nTask: {user_prompt}\nCards:\n{card_text}',
        stage="selection",
    )
    selected = [
        profile
        for profile in _parse_selection(selection, permitted, config["top_k"])
        if profile != "fleetfabric"
    ]
    ...
    for judge in live:
        ...
        try:
            parsed = json.loads(
                _profile_call(judge, user_prompt, stage="ranking", references=candidates)
            )
    ...
    # Two adjacent ranked edges, refined in both directions, keep the graph at
    # four refinement calls and the documented 12-call/7000-token ceiling.
    for left, right in zip(ranked[:2], ranked[1:3]):
        for target, source in ((left, right), (right, left)):
            try:
                refined[target] = _profile_call(
                    target,
                    user_prompt,
                    stage="refinement",
                    own=refined[target],
                    references={source: refined[source]},
                )
            except Exception:
    ...
    pooled = _call(
        meta,
        "Pool these GoA profile outputs into private guidance. Preserve provenance, "
        f"disagreements, and limits.\nTask: {user_prompt}\nOutputs:\n"
        f"{json.dumps(refined, ensure_ascii=False)}",
        stage="pooling",
    )
    degraded = f" Failed nodes: {', '.join(sorted(set(failed)))}." if failed else ""
    return (
        f"[Graph-of-Agents guidance; selected: {', '.join(selected)}; "
        f"12-call/7000-token ceiling.{degraded}]\n{pooled}"
    )


aggregate_goa_context = aggregate_guidance
